 ...
 *
 * - `name="0200000"` BR-KSA-06 subtype literal (simplified credit /
 *   debit note family).
 * - The buyer party is *not* substituted by the template — credit
 *   notes use an empty `<cac:AccountingCustomerParty>` because the
 *   simplified invoice's `buyerName` field is not part of the
 *   `SimplifiedCreditNoteInput` shape in the legacy helper. The
 *   builder still calls `XMLDocument.set` when an explicit
 *   `buyerName` is supplied.
 *
 * Do not reformat.
 */
...
/**
 * Fills the simplified credit note template with values from `input`.
 *
 * `cbc:InvoiceTypeCode` defaults to `388` (tax invoice) when no
 * cancelation is supplied — mirrors the legacy helper, which also
 * fell through to `INVOICE` when the cancellation block was absent
